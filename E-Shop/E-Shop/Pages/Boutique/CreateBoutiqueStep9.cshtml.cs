...
namespace E_Shop.Pages.Boutique
{
    public class CreateBoutiqueStep9Model : PageModel
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    using (var img = Image.FromStream(memoryStream))
                    {
                        ...
                        HttpContext.Session.Set<string>("FileName9", newname);
                    }
                }
                return Page();
            }
        ...
        public async Task<IActionResult> OnPostNext()
        {
            ...
            step9.FileName = HttpContext.Session.Get<string>("FileName9");
            var result = await media.AddBoutiqueMedias(step5.boutiqueId, step9.Lien, step9.Image, step9.Video, step9.Description, step9.FileName);
        ...
        public async Task<IActionResult> OnGet()
        {
            if (HttpContext.Session.Get<Step9>("step9") != null)
            {
                ...
                if (step9.MediaId>0)
                {
                    await media.DeleteMedia(step9.MediaId, _webHostEnvironment);
                }
                HttpContext.Session.Set<Step9>("step9", null);
            }
            return Page();
        }
    }
